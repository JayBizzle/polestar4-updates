...
<div class="wrap">
  <header class="site">
    <div class="eyebrow">Unofficial tracker · UK</div>
    <h1>Polestar 4 <b>Software Updates</b></h1>
    <p class="lede">When the next over-the-air update is likely to land, how often they arrive, and what each one changed. Tracking the <strong>UK</strong> market — version notes from
      <a href="${data.meta.authoritative_source}" target="_blank" rel="noopener">Polestar's UK manual</a>.</p>
  </header>
...
function render(){
  ...
  document.getElementById('banner').innerHTML = \`
    <div>
      ...
      <div class="date">\${fmt(m.predicted)}</div>
      <div class="sub">Last update (\${esc(m.last.version)}) was about \${plural(m.sinceLast,'day')} ago. Updates arrive roughly every \${Math.round(m.avg)} days.</div>
    </div>
  ...
  const gapLabel = (u, older) => {
    if(!u.d) return '';
    if(!older) return '<span class="gap zero">first dated</span>';
    const g = daysBetween(older.d, u.d);
    if(g === 0) return '<span class="gap zero">same day</span>';
    return \`<span class="gap">+\${plural(g,'day')}</span>\`;
  };
  const dateLabel = (u) => {
  ...
  document.getElementById('footer').innerHTML = \`
    ...
    <p>Version notes sourced from the <a href="\${DATA.meta.authoritative_source}" target="_blank" rel="noopener">Polestar 4 manual</a>.
       Data captured \${fmt(parse(DATA.meta.scraped_on))}. Unofficial; not affiliated with Polestar.</p>\`;
}
render();
</script>
</body>

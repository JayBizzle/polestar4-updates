...
`.github/workflows/update.yml` runs hourly (xx:23 UTC) + manual: scrape → on guard failure file a `⚠️` issue (deduped: skipped while one is already open) and fail; on change commit `data.json`+`index.html`+`feed.xml` (auto-deploys via Pages) and on a new version file a `🔔` issue (mentions+assigns the owner so the GitHub app pushes it). Two ntfy pushes (dormant until the `NTFY_TOPIC` secret is set): a loud one on a **new version** (`new_versions`), and a quieter one when an **existing version's notes are edited** (`changed_notes`, body = `notes-change.md` diff). It reads `changed`/`new_versions`/`changed_notes`/`commit_message` from `scrape.js`'s `$GITHUB_OUTPUT`.

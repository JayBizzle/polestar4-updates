 ...
 * Returns { data, changed, newVersions, notesChanged, releasedVersions,
 * upcomingChanged }. `changed` reflects the `updates` array and the upcoming
 * list only (other meta excluded), so no-op days don't churn the file.
 * `notesChanged` lists existing (non-new) versions whose notes content changed,
 * each as { version, added[], removed[] } — a pure reorder is not counted.
 * `releasedVersions` lists existing versions whose prerelease flag flipped off
 * this run (now on the public manual page — the rollout went official).
 * `upcomingChanged` is { added[], removed[] } of upcoming entries vs the stored
 * list; a relabelled build (same internal_version, new carVersion) shows up as
...
function mergeData(existing, scraped, runDate, upcoming, websiteVersions) {
  ...
  const releasedVersions = [];    // prerelease -> official flips
  const merged = [];
  ...
  for (const s of scraped) {
    ...
    if (prev) {
      ...
      if (prev.prerelease && !pre) releasedVersions.push(prev.version);
      merged.push({
  ...
  // Diff of the upcoming list, entry = (version label, build number). A build
  // whose label changed (e.g. 26170 losing its "4.3" name) appears as a
  // removal of the old entry plus an addition of the new one.
  const upEntry = u => ({ version: u.version ?? null, internal_version: u.internal_version });
  const upKey = u => `${u.version ?? ''} ${u.internal_version}`;
  const prevKeys = new Set(prevUpcoming.map(upKey));
  const nextKeys = new Set(nextUpcoming.map(upKey));
  const upcomingChanged = {
    added: nextUpcoming.filter(u => !prevKeys.has(upKey(u))).map(upEntry),
    removed: prevUpcoming.filter(u => !nextKeys.has(upKey(u))).map(upEntry),
  };
  ...
  return { data, changed, newVersions, notesChanged, releasedVersions, upcomingChanged };
}
